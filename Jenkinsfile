	        ...
	        stage("deploy the code to tomcat8"){
	            steps{
	                sshagent(['tomcat']){
	                    ...
	                    sh "scp -o StrictHostKeyChecking=no target/myweb.war ec2-user@172.31.44.120:/opt/tomcat9/webapps"
	                    //start and stop the tomcat8
	                    sh "ssh ec2-user@172.31.44.120 /opt/tomcat9/bin/shutdown.sh"
	                    sh "ssh ec2-user@172.31.44.120 /opt/tomcat9/bin/startup.sh"
	                }
	            }
	        }
